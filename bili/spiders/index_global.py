...
class IndexGlobalSpider(scrapy.Spider):
    ...
    custom_settings = {
        ...
        'ITEM_PIPELINES': {
            ...
            'bili.pipelines.ScoresRedisPipeline': None,
        },
        # 'LOG_LEVEL': "DEBUG",
        'LOG_FILE': r'logs\index_global.log',
    ...
    def parse(self, response):
        ...
        for anime in result['list']:
            for key in IndexGlobalItem.fields.keys():
                if key == 'pub_time':
                    l.add_value(key, datetime.date.fromtimestamp(anime.get(key)) if anime.get(
                        key) >= 0 else datetime.date.fromtimestamp(0) + datetime.timedelta(seconds=anime.get(key)))
                elif key == 'update_time':
                    l.add_value(key, datetime.datetime.fromtimestamp(anime.get(key)) if anime.get(
                        key) >= 0 else datetime.datetime.fromtimestamp(0) + datetime.timedelta(seconds=anime.get(key)))
                else:
        ...
        if response.meta.get('flag'):
            for page in range(2, int(result['pages']) + 1):
                yield scrapy.Request(url=self.start_urls[0].format(page=page), meta={'flag': False, },dont_filter=True)
